...
#define MOTO_INTCOMBO_BOX_GET_PRIVATE(obj) \
  ((MotoIntComboBoxPrivate *) ((MotoIntComboBox *) (obj))->priv)

static void  moto_intcombo_box_finalize    (GObject         *object);
static void  moto_intcombo_box_set_property(GObject         *object,
                                             guint            property_id,
                                             const GValue    *value,
                                             GParamSpec      *pspec);
static void  moto_intcombo_box_get_property(GObject         *object,
                                             guint            property_id,
                                             GValue          *value,
                                             GParamSpec      *pspec);

static void  moto_intcombo_box_data_func   (GtkCellLayout   *layout,
...
G_DEFINE_TYPE(MotoIntComboBox, moto_intcombo_box, GTK_TYPE_COMBO_BOX)


#define parent_class moto_intcombo_box_parent_class

static void
moto_intcombo_box_class_init(MotoIntComboBoxClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);

    object_class->set_property = moto_intcombo_box_set_property;
    object_class->get_property = moto_intcombo_box_get_property;
    object_class->finalize     = moto_intcombo_box_finalize;
...
static void
moto_intcombo_box_init(MotoIntComboBox *combo_box)
{
    ...
    combo_box->priv = G_TYPE_INSTANCE_GET_PRIVATE(combo_box,
                                                  MOTO_TYPE_INTCOMBO_BOX,
                                                  MotoIntComboBoxPrivate);

    priv = MOTO_INTCOMBO_BOX_GET_PRIVATE(combo_box);

    store = moto_intstore_new();
    ...
    gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo_box), cell,
                                   "stock_id", MOTO_INTSTORE_STOCK_ID,
                                   "pixbuf",   MOTO_INTSTORE_PIXBUF,
                                   NULL);
    ...
    gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo_box), cell,
                                   "text", MOTO_INTSTORE_LABEL,
                                   NULL);
}

static void
moto_intcombo_box_finalize(GObject *object)
{
    MotoIntComboBoxPrivate *priv = MOTO_INTCOMBO_BOX_GET_PRIVATE (object);
...
static void
moto_intcombo_box_set_property(GObject      *object,
                                guint         property_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
...
static void
moto_intcombo_box_get_property(GObject    *object,
                                guint       property_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
 ...
 * g_object_new (MOTO_TYPE_INTCOMBO_BOX, NULL).
 *
 ...
 **/
GtkWidget *
moto_intcombo_box_new(const gchar *first_label,
                       gint         first_value,
                       ...)
{
    GtkWidget *combo_box;
    ...
    combo_box = moto_intcombo_box_new_valist(first_label, first_value, args);
...
/**
 * moto_intcombo_box_new_valist:
 * @first_label: the label of the first item
 * @first_value: the value of the first item
 * @values: a va_list with more values
 *
 * A variant of moto_intcombo_box_new() that takes a va_list of
 ...
 **/
GtkWidget *
moto_intcombo_box_new_valist (const gchar *first_label,
                               gint         first_value,
                               va_list      values)
{
    GtkWidget    *combo_box;
    ...
    combo_box = g_object_new(MOTO_TYPE_INTCOMBO_BOX, NULL);
...
/**
 * moto_intcombo_box_new_array:
 * @n_values: the number of values
 * @labels:   an array of labels (array length must be @n_values)
 *
 * A variant of moto_intcombo_box_new() that takes an array of labels.
 ...
 **/
GtkWidget *
moto_intcombo_box_new_array(gint         n_values,
                             const gchar *labels[])
{
    GtkWidget    *combo_box;
    GtkListStore *store;
    ...
    for (i = 0; i < n_values; i++)
    {
        ...
        if(labels[i])
        {
            gtk_list_store_append(store, &iter);
            gtk_list_store_set(store, &iter,
                               MOTO_INTSTORE_VALUE, i,
                               MOTO_INTSTORE_LABEL, labels[i],
                               -1);
        }
    }
 ...
 **/
void
moto_intcombo_box_prepend(MotoIntComboBox *combo_box,
                           ...)
{
    GtkListStore *store;
    GtkTreeIter   iter;
    ...
    g_return_if_fail (MOTO_IS_INTCOMBO_BOX (combo_box));
 ...
 **/
void
moto_intcombo_box_append(MotoIntComboBox *combo_box,
                          ...)
{
    GtkListStore *store;
    GtkTreeIter   iter;
 ...
 **/
gboolean
moto_intcombo_box_set_active(MotoIntComboBox *combo_box,
                              gint             value)
{
    GtkTreeModel *model;
    GtkTreeIter   iter;

    g_return_val_if_fail(MOTO_IS_INTCOMBO_BOX(combo_box), FALSE);

    model = gtk_combo_box_get_model(GTK_COMBO_BOX(combo_box));

    if(moto_intstore_lookup_by_value(model, value, &iter))
    {
 ...
 **/
gboolean
moto_intcombo_box_get_active(MotoIntComboBox *combo_box,
                              gint            *value)
{
    GtkTreeIter  iter;
 ...
 **/
gulong
moto_intcombo_box_connect(MotoIntComboBox *combo_box,
                           gint             value,
                           GCallback        callback,
                           gpointer         data)
{
    ...
    g_return_val_if_fail (MOTO_IS_INTCOMBO_BOX (combo_box), 0);

    if(callback)
        handler = g_signal_connect(combo_box, "changed", callback, data);

    if(! moto_intcombo_box_set_active(combo_box, value))
...
/**
 * moto_intcombo_box_set_sensitivity:
 * @combo_box: a #MotoIntComboBox
 ...
 **/
void
moto_intcombo_box_set_sensitivity(MotoIntComboBox        *combo_box,
                                   ...
                                   GDestroyNotify          destroy)
{
    ...
    gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT (combo_box),
                                       ...
                                       func ?
                                       moto_intcombo_box_data_func : NULL,
                                       priv, NULL);
...
static void
moto_intcombo_box_data_func(GtkCellLayout   *layout,
                             GtkCellRenderer *cell,
                             GtkTreeModel    *model,
                             GtkTreeIter     *iter,
                             gpointer         data)
